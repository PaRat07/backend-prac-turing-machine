...
public:
    ...
    void draw(sf::RenderTarget& target, sf::RenderStates states) const override {
        ...
        // net + vals
        {
            ...
            net_texture.create(real_size.x - cell_size.x - 24, real_size.y - cell_size.y - 24);
        ...
        // lines for syms
        {
            sf::RenderTexture vertical_lines;
            vertical_lines.create(real_size.x - cell_size.x - 24, cell_size.y);
            vertical_lines.clear(background_color);

            sf::RectangleShape line(sf::Vector2f(2, cell_size.y));
            line.setFillColor(outline_color);

            for (int i = pos_in_.x % cell_size.x; i < real_size.x; i += cell_size.x) {
                line.setPosition(i, 0);
                vertical_lines.draw(line);
            }
            vertical_lines.display();
            sf::Sprite sprite;
            sprite.setPosition(cell_size.x + 12, cell_size.y + 12);
            sprite.setTexture(vertical_lines.getTexture());
            target.draw(sprite);
        }

        // lines for qs
        {
            sf::RenderTexture horizontal_lines;
            horizontal_lines.create(cell_size.x, real_size.y - cell_size.y - 24);
            horizontal_lines.clear(background_color);
            ...
            for (int i = pos_in_.y % cell_size.y; i < real_size.y; i += cell_size.y) {
                line.setPosition(0, i);
                horizontal_lines.draw(line);
            }
            horizontal_lines.display();
            sf::Sprite sprite;
            sprite.setPosition(12, cell_size.y + 12);
            sprite.setTexture(horizontal_lines.getTexture());
            target.draw(sprite);
        }

        {
            sf::RenderTexture buttons_syms;
            buttons_syms.create(real_size.x - cell_size.x - 24, cell_size.y);
            buttons_syms.clear(sf::Color::Transparent);
            for (int i = -pos_in_.x / cell_size.x; i < machine_.Size().x && i * cell_size.x + pos_in_.x < real_size.x; ++i) {
                char to_delete = machine_.GetSyms()[i][0];
                auto eraser = [&, to_delete] () {
                    machine_.EraseSym(to_delete);
                };
                ButtonWithTextRelativePos erase_sym(sf::Vector2f(i * cell_size.x + pos_in_.x, 0), sf::Vector2f(20, 20), "-", eraser);
                buttons_syms.draw(erase_sym);
            }
            buttons_syms.display();
            sf::Sprite sprite;
            sprite.setPosition(cell_size.x + 1, 12);
            sprite.setTexture(buttons_syms.getTexture());
            target.draw(sprite);
        }
        ...
        // syms
        {
            ...
            syms.create(real_size.x - cell_size.x - 24, cell_size.y);
            ...
            for (int i = -pos_in_.x / cell_size.x; i < syms_vals.size() && i * cell_size.x + pos_in_.x < real_size.x; ++i) {
                ...
                str.setPosition(sf::Vector2f(i * cell_size.x + cell_size.x / 2 + pos_in_.x, cell_size.y / 2));
                syms.draw(str);
            }
            syms.display();
            sf::Sprite sprite;
        ...
        // qs
        {
            ...
            sf::RenderTexture syms;
            syms.create(cell_size.x, real_size.y - cell_size.y - 24);
            syms.clear(sf::Color::Transparent);
            CenterPositionedString str;
        ...
        // ver line(not texture)
        {
            ...
            legend_border.setPosition(sf::Vector2f(pos_.x * target.getSize().x + 12, pos_.y * target.getSize().y + 12 + cell_size.y));
            target.draw(legend_border);
        }
    ...
    bool ProcessEvent(sf::Event event) override {
        switch (event.type) {
            case sf::Event::EventType::MouseWheelScrolled: {
                ...
                return std::abs(pos_in_.x - was_pos_in_.x) > 2 || std::abs(pos_in_.y - was_pos_in_.y) > 2;
            }
            case sf::Event::EventType::MouseButtonPressed: {
                ...
                if (std::abs(pos_.x + size_.x / 2 - pos.x) <= size_.x / 2 &&
                    ...
                    active_pos_.emplace((event.mouseButton.x - pos_in_.x - 14) / cell_size.x - 1,
                                        (event.mouseButton.y - pos_in_.y - 14) / cell_size.y - 1);
            ...
            case sf::Event::TextEntered:
                if (field.has_value()) {
                    if (event.text.unicode == 13) {
                        ...
                        return field->ProcessEvent(event);
                    }
                }
                break;
        }
...
private:
    ...
    mutable sf::Vector2i was_pos_in_ = {0,0};
